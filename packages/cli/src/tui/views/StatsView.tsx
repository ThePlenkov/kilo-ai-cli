...
export function StatsView({ token, onBack, focused }: StatsViewProps) {
  ...
  return (
    <Box flexDirection="column">
      <Box marginBottom={1}><Text bold color="cyan">Security Statistics</Text></Box>

      {/* Severity breakdown */}
      <Box flexDirection="column" marginBottom={1} borderStyle="single" borderColor="red" paddingX={1}>
        <Text bold color="red">By Severity</Text>
        <StatBar label="Critical" value={Number(critical) || 0} total={Number(total) || 0} color="red" />
        <StatBar label="High" value={Number(high) || 0} total={Number(total) || 0} color="yellow" />
        <StatBar label="Medium" value={Number(medium) || 0} total={Number(total) || 0} color="blue" />
      ...
      <Box flexDirection="column" marginBottom={1} borderStyle="single" borderColor="green" paddingX={1}>
        <Text bold color="green">By Status</Text>
        <StatBar label="Open" value={Number(open) || 0} total={Number(total) || 0} color="red" />
        <StatBar label="Fixed" value={Number(fixed) || 0} total={Number(total) || 0} color="green" />
        <StatBar label="Ignored" value={Number(ignored) || 0} total={Number(total) || 0} color="gray" />
      </Box>

      {/* Raw values */}
      <Box flexDirection="column" marginTop={1}>
        <Text bold dimColor>All values:</Text>
        {entries.map(([k, v]) => (
          <Box key={k}>
...
function StatBar({ label, value, total, color }: { label: string; value: number; total: number; color: string }) {
  ...
  return (
    <Box>
      <Box width={10}><Text>{label}</Text></Box>
