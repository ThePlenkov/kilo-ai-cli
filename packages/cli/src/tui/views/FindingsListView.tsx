    ...
    <Box flexDirection="column">
      ...
      <Box marginBottom={1}>
        <Text>
          ...
          <Text dimColor>
            {' '}
            | Running: {running} | Concurrency: {concurrency}
          </Text>
        </Text>
      </Box>
      ...
      <Box flexDirection="column">
        {visibleFindings.map((f, i) => {
          ...
          return <FindingRow key={f.id} finding={f} selected={idx === selectedIdx} />
        })}
      </Box>

      {/* Scroll indicator below */}
      {hasMoreBelow ? (
        <Text dimColor> ↓ {findings.length - scrollOffset - maxVisible} more below</Text>
      ) : null}

      {/* Position indicator */}
      <Box marginTop={1}>
        <Text dimColor>
          [{selectedIdx + 1}/{findings.length}] Up/Down navigate Enter=details f=severity s=status
          n=next page p=prev r=refresh Esc=back
        </Text>
      </Box>
