...
export const kiloclawSubscriptionDetailCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    if (detail.sandboxId) console.log(`Sandbox ID: ${sanitize(detail.sandboxId)}`)
    if (detail.activationState) console.log(`Activation state: ${sanitize(detail.activationState)}`)
    if (detail.priceVersion) console.log(`Price version: ${sanitize(detail.priceVersion)}`)
    if (detail.selfServiceInstanceType) console.log(`Instance type: ${sanitize(detail.selfServiceInstanceType)}`)
...
export const kiloclawVersionCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    if (result.imageDigest) console.log(`Image digest: ${sanitize(result.imageDigest)}`)
