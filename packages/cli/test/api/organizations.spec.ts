...
describe('organizations API', () => {
  ...
  it('getOrganizationWithMembers passes organizationId', async () => {
    fetchMock.mockResolvedValue(mockResponse({ id: 'o1', name: 'Org', role: 'owner', members: [{ id: 'm1', email: 'user@test.com', role: 'admin' }] }))
  ...
  it('listChildOrganizations passes organizationId', async () => {
    ...
    expect(decodeURIComponent((fetchMock.mock.calls[0]![0] as string).split('input=')[1]!)).toContain('o1')
  })

  it('getOrganizationUsageStats returns stats', async () => {
    fetchMock.mockResolvedValue(mockResponse({ totalCreditsUsed: 100, creditsUsedThisPeriod: 50, activeSessions: 3, totalMembers: 5 }))
  ...
  it('getCreditTransactions returns transactions', async () => {
    fetchMock.mockResolvedValue(mockResponse([{ id: 't1', amount: 10, type: 'credit', description: 'test', createdAt: '2024-01-01' }]))
  ...
  it('getOrganizationInvoices passes period', async () => {
    fetchMock.mockResolvedValue(mockResponse([{ id: 'i1', date: '2024-01-01', amount: 100, status: 'paid' }]))
  ...
  it('createOrganization posts with name and domain', async () => {
    ...
    const result = await createOrganization('tok', { name: 'New Org', companyDomain: 'example.com' })
    expect(result.id).toBe('o1')
    const init = fetchMock.mock.calls[0]![1] as { body: string }
    expect(JSON.parse(init.body)).toEqual({ '0': { name: 'New Org', companyDomain: 'example.com' } })
  })
  ...
  it('listAvailableModels calls organizations.settings.listAvailableModels', async () => {
    fetchMock.mockResolvedValue(mockResponse([{ id: 'm1', name: 'gpt-4', provider: 'openai', isEnabled: true }]))
  ...
  it('getSecurityAgentPermissionStatus calls organizations.securityAgent.getPermissionStatus', async () => {
    fetchMock.mockResolvedValue(mockResponse({ granted: true, permissions: ['read'], pendingRequests: 0 }))
